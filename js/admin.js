(() => {
  ...
  function getCGPAs() {
    return [...document.querySelectorAll(".cgpa-input")].map(x => Number(x.value)).filter(x => Number.isFinite(x) && x >= 0 && x <= 4);
  }

  function gradeFor(cgpa) {
    if (cgpa >= 3.75) return ["A+", "Excellent"];
    if (cgpa >= 3.50) return ["A", "Very Good"];
    if (cgpa >= 3.25) return ["A-", "Good"];
    if (cgpa >= 3.00) return ["B+", "Good"];
    if (cgpa >= 2.75) return ["B", "Satisfactory"];
    if (cgpa >= 2.50) return ["B-", "Satisfactory"];
    if (cgpa >= 2.25) return ["C", "Average"];
    if (cgpa >= 2.00) return ["D", "Pass"];
    return ["F", "Fail"];
  }
  ...
  function updateSummary() {
    ...
    const [grade, text] = values.length ? gradeFor(overall) : ["—", "Waiting for CGPA"];
    ...
    document.getElementById("overallResult").className = `mt-2 text-4xl font-black ${result === "PASS" ? "text-emerald-600" : result === "FAIL" ? "text-red-600" : "text-slate-700"}`;
  }
  ...
  function buildResultText() {
    ...
    const semesterLines = values.map((v,i) => `${i+1}${i===0?"st":i===1?"nd":i===2?"rd":"th"} Semester : ${v.toFixed(2)}`).join("\n");
    return [
      "RESULTGUARD",
  ...
  document.getElementById("generate").addEventListener("click", async () => {
    ...
    const studentName = document.getElementById("studentName").value.trim();
    const values = getCGPAs();
    if (!studentId || !studentName || !values.length || values.length !== document.querySelectorAll(".cgpa-input").length) {
    ...
    try {
      ...
      console.error(e);
      toast("Cryptographic signing failed. Run the project through a local server.");
    }
  });
